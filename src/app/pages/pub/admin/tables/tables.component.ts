...
export class TablesComponent {
  ...
  // Propriedades para gerenciamento de mesas
  showModal = false;
  ...
  removeProduct(index: number) {
    if (this.selectedTable && this.selectedTable.products) {
      ...
      this.selectedTable.products.splice(index, 1);
      
      // Recalcula o subtotal após a remoção
      this.calculateSubtotal();
      
      // Se não há mais produtos, pode opcionalmente liberar a mesa
      if (this.selectedTable.products.length === 0) {
        // Opcional: perguntar se deseja liberar a mesa
        // Por enquanto, apenas mantém a mesa ocupada mas sem produtos
      }
    }
  }
